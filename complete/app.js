...
function invoiceSections(full){
  ...
  result.push({title:"Authorization (optional)",fields:[["Authorized signatory","[NAME / TITLE]"],["Signature","[SIGNATURE]"],["Date","[DATE]"]]});
  return result;
}
...
const draftSchemaVersion=id=>id==="offer-letter"?4:id==="invoice-full"?2:1;
...
function restoreDraft(root,id){try{const saved=JSON.parse(localStorage.getItem(draftKey(id))||"null");if(saved?.html){root.innerHTML=cleanDraftHtml(saved.html);if(id==="offer-letter"&&root.classList.contains("offer-document")){
    ...
    if(id==="offer-letter"&&(saved.schemaVersion||1)<2){const before=[...root.querySelectorAll(".doc-section")].find(node=>$("h2 .editable",node)?.textContent.trim()==="Conditions before joining")||$(".doc-footer",root);for(const title of ["Standard workplace rules","Policies, changes & separation"]){const exists=[...root.querySelectorAll(".doc-section h2 .editable")].some(node=>node.textContent.trim()===title);if(!exists){const section=makeSection(details[id].sections.find(item=>item.title===title));root.insertBefore(section,before)}}toast("Saved draft restored with new rules. Save Edits to keep them.")}
    else if(id==="invoice-full"&&(saved.schemaVersion||1)<2){const title="Authorization (optional)",exists=[...root.querySelectorAll(".doc-section h2 .editable")].some(node=>node.textContent.trim()===title);if(!exists)root.insertBefore(makeSection(details[id].sections.find(item=>item.title===title)),$(".doc-footer",root));toast("Saved invoice restored with optional signature and stamp. Save Edits to keep them.")}
    else toast("Saved draft restored on this device.")}}catch(error){console.warn("Could not restore draft",error)}}
function mediaSlot(kind){
  const slot=el("div",`media-slot media-${kind}`),preview=el("img","media-preview");
  slot.dataset.mediaKind=kind;preview.alt=kind==="stamp"?"Company stamp":"Signature";preview.hidden=true;
  const actions=el("div","media-actions"),upload=el("button","media-upload",kind==="stamp"?"Add stamp":"Add signature"),remove=el("button","media-remove","Remove");
  upload.type=remove.type="button";remove.hidden=true;actions.append(upload,remove);slot.append(preview,actions);return slot;
}
function syncMediaSlot(slot){
  const present=!!$(".media-preview",slot)?.getAttribute("src");
  slot.classList.toggle("has-image",present);$(".media-preview",slot).hidden=!present;
  $(".media-upload",slot).textContent=`${present?"Change":"Add"} ${slot.dataset.mediaKind}`;
  $(".media-remove",slot).hidden=!present;
  if(slot.dataset.mediaKind==="signature")slot.closest(".signature-field,.offer-signature-line")?.classList.toggle("has-signature-image",present);
}
function ensureMediaSlots(root){
  for(const field of root.querySelectorAll(".field")){
    const label=$("label",field)?.textContent.trim()||"";
    if(!/signature/i.test(label))continue;
    field.classList.add("signature-field");
    if(!$(".media-signature",field))field.insertBefore(mediaSlot("signature"),field.querySelector(":scope > .unit-remove"));
    if(/provider signature|employer signature|signature and stamp|^signature$/i.test(label)&&!$(".media-stamp",field))field.insertBefore(mediaSlot("stamp"),field.querySelector(":scope > .unit-remove"));
  }
  root.querySelectorAll(".offer-signature-box").forEach((box,index)=>{
    const line=$(".offer-signature-line",box);if(!line)return;
    if(!$(".media-signature",line))line.append(mediaSlot("signature"));
    if(index===1&&!$(".media-stamp",box))line.after(mediaSlot("stamp"));
  });
  root.querySelectorAll(".media-slot").forEach(syncMediaSlot);
}
...
function wireEditor(){
  ...
  root.addEventListener("click",event=>{
    ...
    const mediaUpload=event.target.closest(".media-upload"),mediaRemove=event.target.closest(".media-remove");
    if(mediaUpload){activeMediaSlot=mediaUpload.closest(".media-slot");$("#media-input")?.click();return}
    if(mediaRemove){const slot=mediaRemove.closest(".media-slot");$(".media-preview",slot).removeAttribute("src");syncMediaSlot(slot);toast("Image removed. Save Edits to keep this change.");return}
  ...
  $("#media-input")?.addEventListener("change",uploadMedia);
...
async function mediaImageData(file){
  ...
  if(file.size<=550000&&!/heic|heif/.test(file.type))return await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=()=>reject(Error("Could not read image."));reader.readAsDataURL(file)});
  ...
  try{
    ...
    for(const maxSide of [1000,700,500]){
      ...
      for(const format of [file.type==="image/png"?"image/png":"image/jpeg","image/webp"]){const data=canvas.toDataURL(format,.78);if(data.length<=800000)return data}
    }
    throw Error("Image is too detailed. Try a closer photo or a smaller image.");
  }finally{URL.revokeObjectURL(url)}
}
async function uploadMedia(event){
  const file=event.target.files?.[0],slot=activeMediaSlot;event.target.value="";activeMediaSlot=null;
...
async function saveStandalone(name,id){try{
    ...
    const html=`<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(name)} — Client Templates</title><style data-kit-style>${css.replace(/<\/style/gi,"<\\/style")}</style></head><body data-page="standalone" class="${light.trim()}"><div class="editor-toolbar" role="toolbar" aria-label="Document tools"><span class="back-link">CLIENT TEMPLATES</span><div class="toolbar-actions"><button id="undo-button" class="tool-button muted-button" type="button">Undo</button><button id="logo-button" class="tool-button" type="button">Add logo</button><button id="add-section-button" class="tool-button" type="button">+ Section</button><button id="theme-button" class="tool-button muted-button" type="button">${light?"Dark mode":"Light mode"}</button><button id="save-button" class="tool-button" type="button">Save Edits</button><button id="download-button" class="tool-button" type="button">Download HTML</button><button id="pdf-button" class="tool-button primary-button" type="button">Save as PDF</button><button id="png-button" class="tool-button primary-button" type="button">Save as PNG</button></div></div><input id="logo-input" type="file" accept="image/png,image/jpeg,image/webp" hidden><input id="media-input" type="file" accept="image/png,image/jpeg,image/webp,image/heic,image/heif" hidden><main class="editor-shell"><article id="document" class="${escapeHtml(clone.className)}" data-template="${escapeHtml(id)}">${clone.innerHTML}</article></main><div id="toast" class="toast" role="status" aria-live="polite"></div><script data-kit-capture>${capture.replace(/<\/script/gi,"<\\/script")}<\/script><script data-kit-runtime>${js.replace(/<\/script/gi,"<\\/script")}<\/script></body></html>`;
