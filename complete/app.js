...
function renderOfferLetter(root){
  ...
  function page(number){
    const sheet=el("section","offer-page"),head=el("header","offer-letterhead"),brand=el("div","offer-letter-brand"),logo=el("img","company-logo");
    logo.alt="Company logo";logo.hidden=true;
    const name=el("div","offer-company-name");name.append(editable("[COMPANY NAME]"));
    brand.append(logo,removable(name,"company name"));
    head.append(brand,el("div","offer-letterhead-rule"));
    const body=el("div","offer-page-body"),foot=el("footer","offer-page-footer");
    foot.append(editable("[COMPANY ADDRESS]"),document.createTextNode("  |  "),editable("[COMPANY WEBSITE]"));
  ...
  const signatures=el("div","offer-signatures");for(const [title,name,role] of [["Candidate acknowledgement","[CANDIDATE NAME]","[SIGNATURE / DATE]"],["For the employer","[AUTHORISED SIGNATORY]","[NAME / TITLE / DATE]"],["Human resources","[HR CONTACT]","[NAME / TITLE / DATE]"]]){const box=el("div","offer-signature-box");box.append(el("div","offer-signature-line"),paragraph(title),paragraph(name),paragraph(role));signatures.append(box)}second.append(signatures);
...
const draftSchemaVersion=id=>id==="offer-letter"?3:1;
...
function wireEditor(){
  ...
  $("#theme-button")?.addEventListener("click",()=>{document.body.classList.toggle("light");const light=document.body.classList.contains("light");$("#theme-button").textContent=light?"Dark mode":"Light mode";$("#theme-button").setAttribute("aria-pressed",String(light));try{localStorage.setItem("client-kit-theme",light?"light":"dark")}catch(error){}});
...
async function saveStandalone(name,id){try{
    ...
    const url=URL.createObjectURL(new Blob([html],{type:"text/html"})),link=el("a");link.href=url;link.download=`client-kit-${id}-edited.html`;document.body.append(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),30000);toast("Editable HTML downloaded.")
